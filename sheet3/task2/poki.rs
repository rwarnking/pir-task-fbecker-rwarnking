...
fn main() {
    ...
    println!("You have chosen {}!", red.name);
    ...
    while !finished {
        println!(">>>> Status: {} has {} HP, {} has {} HP", red_pokemon.name(), red_pokemon.stats().hp, blue_pokemon.name(), blue_pokemon.stats().hp,);
...
fn print_pokemon_list() {
    for pokemon in POKEDEX {
        println!("#{number:>0width$} {name}" , number=pokemon.id, width = 3, name = pokemon.name);
    }
}
